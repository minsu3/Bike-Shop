...
db.serialize(() => {
  ...
  for (let user of users) {
    ...
    db.run(insertUsersQuery, userData, (error) => {
      if (error)
        ...
        console.log(
          `Inserted user with name ${user.first_name} ${user.last_name} into 'users' table`
        );
    });
  }

  // Drop, Create, Insert Bikes Makes list and table
  const dropBikeMakeTableQuery =
    "DROP TABLE IF EXISTS bike_make";
  db.run(dropBikeMakeTableQuery, (error) => {
    if (error)
      console.error("Error dropping 'bike_make' table");
  ...
  db.run(createBikeMakeTableQuery, (error) => {
    if (error)
      console.error("Error creating 'bike_make' table");
    else console.log("Created 'bike_make' table");
  });

  const insertBikeMakeQuery =
    "INSERT INTO bike_make VALUES (?, ?, ?)";
  for (let bikemake of bikes_make_list) {
    ...
    db.run(insertBikeMakeQuery, bikeMakeData, (error) => {
      if (error)
        console.log(
          "Could not insert into bike_make table",
          [bikemake.brand, bikemake.type, bikemake.category],
          error
        );
  ...
  const dropBikeDetailsTableQuery =
    "DROP TABLE IF EXISTS bike_details";
  db.run(dropBikeDetailsTableQuery, (error) => {
    if (error)
      console.error("Error dropping 'bike_details' table");
  ...
  db.run(createBikeDetailsTableQuery, (error) => {
    if (error)
      console.error("Error creating 'bike_details' table");
    else console.log("Created 'bike_details' table");
  });

  const insertBikeDetailsQuery =
    "INSERT INTO bike_details VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
  for (let bikedetail of bike_details_list) {
    let bikeDetailData = [
      bikedetail.brand,
      bikedetail.bike_name,
      bikedetail.image,
      bikedetail.price,
      bikedetail.size,
      bikedetail.color,
      bikedetail.material,
      bikedetail.serial_number,
      bikedetail.component,
    ];
    db.run(
      insertBikeDetailsQuery,
      bikeDetailData,
      (error) => {
        if (error)
          console.log(
            "Could not insert into bike_details table",
            [
              bikedetail.brand,
              bikedetail.bike_name,
              bikedetail.image,
          ...
          console.log(
            `Inserted bike into bike_details table`
          );
      }
    );
  }
  ...
  const createOrdersTableQuery =
    "CREATE TABLE orders (users_id INTEGER, bikespec_id INTEGER)";
  db.run(createOrdersTableQuery, (error) => {
    if (error)
      console.error("Error creating 'orders' table");
